...
from backend.admin_ops import do_request, send_request, signed_request
...
@pytest.mark.anyio
async def test_do_request(mocker: MockerFixture) -> None:
    p = mocker.patch("backend.admin_ops.send_request")
    await do_request(
        url="foo.bar",
        access_key="foo",
        secret_key="bar",
    ...
    args = p.call_args_list.pop()
    req: httpx.Request = args.args[0]
    assert req.method == "GET"
    assert req.url == "foo.bar/asd"


@pytest.mark.anyio
async def test_send_request(mocker: MockerFixture) -> None:
    p = mocker.patch("httpx.AsyncClient.send")
    req = httpx.Request("GET", "foo.bar")
    await send_request(req)
    p.assert_called_once_with(req)
